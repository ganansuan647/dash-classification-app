...
def serve_prediction_confidence_plot(model, X_train, X_test, y_train, y_test, mesh_step, margin):
    # Create a mesh grid on which we will run our model
    l_min, l_max = X_train[:, 0].min() - margin, X_train[:, 0].max() + margin
    w_min, w_max = X_train[:, 1].min() - margin, X_train[:, 1].max() + margin
    lrange = np.arange(l_min, l_max, mesh_step)
    wrange = np.arange(w_min, w_max, mesh_step)
    ...
    fig = px.scatter(
        ...
        symbol=y_test,
        symbol_map={0: 'square-dot', 1: 'circle-dot', 2: 'diamond-dot'},
    )
    fig.update_traces(
    ...
    fig.add_trace(
        go.Heatmap(
            ...
            hovertemplate=(
                'Feature 1: %{x} <br>'
                'Feature 2: %{y} <br>'
                'p(Class 0): %{customdata[0]:.3f}<br>'
                'p(Class 1): %{customdata[1]:.3f}<br>'
                'p(Class 2): %{customdata[2]:.3f}<extra></extra>'
            )
        )
    )
    fig.update_layout(
